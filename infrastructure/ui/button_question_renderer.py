...
class ButtonQuestionRenderer(SequenceQuestionRendererProtocol):
    ...
    async def render_completion_message(
        ...
        if sequence_definition.is_single_question():
            return self._render_single_question_completion(session, completion_template, user)
        
        # For multi-question sequences, generate summary
        return self._render_multi_question_completion(session, sequence_definition, completion_template, user)
    ...
    def _create_choice_keyboard(
        ...
        for option in question.options:
            ...
            callback_data = f"seq_answer:{session.sequence_name}:{question.key}:{option.value}"
    ...
    def _render_multi_question_completion(
        ...
        for question in sequence_definition.questions:
            ...
            if answer:
                ...
                display_value = self._get_display_value(question, answer.answer_value, user)
                question_text = question.question_text or self._get_localized_text(question.question_text_key, '', 'Question', user)
                message += f"• {question_text} {display_value}\n"
        
        return message
    
    def _get_display_value(self, question: SequenceQuestion, answer_value: str, user: Optional[User] = None) -> str:
        """
    ...
    def _get_localized_text(self, key: Optional[str], fallback: Optional[str], default: str, user: Optional[User] = None) -> str:
        """
