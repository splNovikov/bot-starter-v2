...
class HandlersRegistry:
    ...
    def register(
        ...
        if expected_protocol and not validate_handler_signature(
            handler_func, expected_protocol
        ):
            raise ValueError(
        ...
        registered_handler = RegisteredHandler(
            function=handler_func, metadata=metadata, stats=HandlerStats()
        )
        ...
        self._logger.info(
            f"Registered handler: {handler_id} ({metadata.handler_type.value})"
        )
        return handler_id
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                handler.stats.avg_response_time = (
                    handler.stats.avg_response_time * (handler.stats.calls - 1)
                    + duration
                ) / handler.stats.calls
    ...
    def get_handlers_by_category(
        self, category: HandlerCategory
