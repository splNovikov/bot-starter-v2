...
class SequenceService(SequenceServiceProtocol):
    ...
    def start_sequence(
        ...
        sequence_definition = self._sequence_provider.get_sequence_definition(sequence_name)
        ...
        if session:
            ...
            if sequence_definition.scored:
                session.max_possible_score = sequence_definition.get_total_possible_score()
    ...
    def process_answer(
        ...
        sequence_definition = self._sequence_provider.get_sequence_definition(session.sequence_name)
        if sequence_definition and sequence_definition.scored and current_question.correct_answer:
            answer.is_correct = self._check_correct_answer(current_question, answer_text)
        ...
        next_question_key = self._sequence_provider.get_next_question_key(updated_session, user)
        
        # Check if sequence is complete
        if not next_question_key:
            self._session_manager.complete_session(user_id)
            logger.info(f"Completed sequence {session.sequence_name} for user {user_id}")
    ...
    async def send_question(
        ...
        try:
            ...
            logger.error(f"Error sending question {question_key} to user {user.id}: {e}")
    ...
    async def send_completion_message(
        ...
        try:
            ...
            logger.info(f"Sent completion message for sequence {session.sequence_name} to user {user.id}")
            return True
    ...
    def _get_current_question(self, session: SequenceSession) -> Optional[SequenceQuestion]:
        """Get current question for session."""
        sequence_definition = self._sequence_provider.get_sequence_definition(session.sequence_name)
        if not sequence_definition:
            return None
        
        return self._sequence_provider.get_current_question(session.sequence_name, session.current_step)
    
    def _check_correct_answer(self, question: SequenceQuestion, answer_text: str) -> bool:
        ...
        if isinstance(question.correct_answer, list):
            ...
            return answer_text.lower().strip() in [a.lower().strip() for a in question.correct_answer]
        else:
    ...
    async def _default_render_completion(
        ...
        base_message = t("sequence.completion.generic", 
                        user=user, 
                        sequence_type=sequence_name)
        
        # Add scoring information for scored sequences
        if sequence_definition and sequence_definition.scored and session.total_score is not None:
