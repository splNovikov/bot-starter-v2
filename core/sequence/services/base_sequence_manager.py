...
from ..types import SequenceSession, SequenceAnswer, SequenceStatus
...
class BaseSequenceManager(SequenceManagerProtocol, ABC):
    ...
    def add_answer(self, user_id: int, answer: SequenceAnswer) -> bool:
        ...
        try:
            ...
            logger.debug(f"Added answer for {answer.question_key} to session {session.session_id}")
            return True
    ...
    def get_active_sessions_count(self) -> int:
        ...
        return len([s for s in self._sessions.values() if s.status == SequenceStatus.ACTIVE])
    
    def get_completed_sessions_count(self) -> int:
        """Get count of completed sessions."""
        return len([s for s in self._sessions.values() if s.status == SequenceStatus.COMPLETED])
    ...
    def cleanup_abandoned_sessions(self, max_age_hours: int = 24) -> int:
        ...
        for user_id, session in self._sessions.items():
            if (session.status in [SequenceStatus.ABANDONED, SequenceStatus.COMPLETED] and 
