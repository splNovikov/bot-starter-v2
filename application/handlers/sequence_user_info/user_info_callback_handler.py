...
async def user_info_callback_handler(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        if question_key == "confirm_user_name" and answer_value.lower() == "true":
            logger.info(
                f"User {callback.from_user.id} confirmed their name, saving 'John_Doe' as preferred_name"
            )
            ...
            if user_service:
                try:
                    ...
                    logger.debug(
                        f"Attempting to update user {callback.from_user.id} with preferred_name: John_Doe"
                    )
                    updated_user = await user_service.update_user(
                        callback.from_user, {"preferred_name": "John_Doe"}
                    )

                    if updated_user:
                        logger.info(
                            f"Successfully saved preferred_name 'John_Doe' for user {callback.from_user.id}"
                        )
                        logger.debug(f"Updated user data: {updated_user}")
                    else:
                        logger.error(
                            f"Failed to save preferred_name for user {callback.from_user.id} - update_user returned None"
                        )
                except Exception as e:
                    logger.error(
                        f"Error saving preferred_name for user {callback.from_user.id}: {e}"
                    )
