...
async def handle_user_info(message: Message):
    ...
    if not sequence_service:
        await message.answer(
            "❌ Sequence service is not available. Please try again later."
        )
        return
    ...
    try:
        ...
        if next_question_key:
            ...
            await message.answer(
                "❌ Failed to start user info sequence. Please try again."
            )

    except Exception as e:
        logger.error(
            f"Error starting user info sequence for user {message.from_user.id}: {e}"
        )
        await message.answer(
            "❌ An error occurred while starting the sequence. Please try again."
        )
